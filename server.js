...
function autoAssignLead(s,clientId,l){
  const cfg=s.realEstate?.automation||{}; if(cfg.autoAssign===false || l.assignedTo) return null;
...
function seedLeadFollowups(s,clientId,l){
  const cfg=s.realEstate?.automation?.followups||[]; if(!cfg.length) return;
...
async function processRealEstateAutomation(){
  try{
    ...
    for(const l of s.leads){
      const cfg=s.realEstate.automation||{}; const age=(now-new Date(l.date||now).getTime())/60000;
      if(l.clientId && l.pipelineStage==="NEW" && age>=Number(cfg.missedLeadMinutes||10) && !l.missedLeadAt){l.missedLeadAt=reNow();activity(s,l.clientId,"alert",`Missed lead alert: ${l.name} has not been contacted`,{leadId:l.id});changed=true;}
      if(l.clientId && l.pipelineStage==="NEW" && age>=Number(cfg.escalationMinutes||30) && !l.escalatedAt){l.escalatedAt=reNow();activity(s,l.clientId,"escalation",`Escalation: ${l.name} is still uncontacted after ${Math.round(age)} minutes`,{leadId:l.id});changed=true;}
    }
    const dueFollowups=s.realEstate.followups.filter(f=>f.status==="pending"&&new Date(f.dueAt).getTime()<=now&&!f.queuedAt);
...
app.post("/api/whatsapp/send-template",requireAuth,async(req,res)=>{try{const to=String(req.body.to||"");const template=String(req.body.template||"").trim();const language=String(req.body.language||"en_US");const params=Array.isArray(req.body.params)?req.body.params:[];if(!to||!template)return res.status(400).json({error:"to and template are required"});const out=await sendWhatsAppTemplate(to,template,language,params);res.json({ok:true,...out});}catch(e){res.status(502).json({error:e.message});}});
...
app.get("/api/realestate/automation",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);const a=s.realEstate.automation||{};const leads=s.leads.filter(x=>x.clientId===id);const follow=s.realEstate.followups.filter(x=>x.clientId===id);const uncontacted=leads.filter(x=>x.pipelineStage==="NEW");const queued=follow.filter(x=>x.status==="queued");const wa=waConfig();res.json({config:a,waAutomatedFollowups:!!(wa.phoneNumberId&&wa.accessToken&&wa.followupTemplate),uncontacted:uncontacted.length,missed:uncontacted.filter(x=>x.missedLeadAt).length,escalated:uncontacted.filter(x=>x.escalatedAt).length,followups:{pending:follow.filter(x=>x.status==="pending").length,queued:queued.length,sent:follow.filter(x=>x.status==="sent").length,completed:follow.filter(x=>x.status==="completed").length,failed:follow.filter(x=>x.status==="queued"&&x.lastSendError).length},queue:queued.slice(0,100)});}catch(e){res.status(403).json({error:e.message});}});
app.post("/api/realestate/automation",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);const a=s.realEstate.automation||{};if(req.body.missedLeadMinutes!==undefined)a.missedLeadMinutes=Math.max(1,Number(req.body.missedLeadMinutes)||10);if(req.body.escalationMinutes!==undefined)a.escalationMinutes=Math.max(a.missedLeadMinutes+1,Number(req.body.escalationMinutes)||30);if(req.body.autoAssign!==undefined)a.autoAssign=!!req.body.autoAssign;if(Array.isArray(req.body.followups))a.followups=req.body.followups.slice(0,20).map(x=>({id:String(x.id||reId("rule")),day:Math.max(0,Number(x.day)||0),stage:String(x.stage||"lead"),channel:String(x.channel||"whatsapp"),message:String(x.message||"").slice(0,1000)})).filter(x=>x.message);s.realEstate.automation=a;activity(s,id,"automation","Automation settings updated");writeStore(s);res.json(a);}catch(e){res.status(403).json({error:e.message});}});
app.post("/api/realestate/automation/run",requireAuth,(req,res)=>{try{processRealEstateAutomation();const s=ensureStoreShape(readStore()),id=reClient(req,s);res.json({ok:true,clientId:id,ranAt:reNow()});}catch(e){res.status(403).json({error:e.message});}});
...
app.get("/api/realestate/documents",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);const leadId=req.query.leadId;res.json(s.realEstate.documents.filter(x=>x.clientId===id&&(!leadId||x.leadId===leadId)));}catch(e){res.status(403).json({error:e.message});}});
app.post("/api/realestate/documents",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);const l=req.body.leadId?leadById(s,req.body.leadId,id):null;const x={id:reId("doc"),clientId:id,leadId:req.body.leadId||null,type:String(req.body.type||"Other").slice(0,60),label:String(req.body.label||"").slice(0,140),url:String(req.body.url||"").slice(0,500),status:String(req.body.status||"received"),createdAt:reNow()};if(!x.label)return res.status(400).json({error:"A label/document name is required"});s.realEstate.documents.push(x);activity(s,id,"document",`${x.type} recorded${l?" for "+l.name:""}`,{leadId:x.leadId,docId:x.id});writeStore(s);res.status(201).json(x);}catch(e){res.status(403).json({error:e.message});}});
app.patch("/api/realestate/documents/:id",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s),x=s.realEstate.documents.find(d=>d.id===req.params.id&&d.clientId===id);if(!x)return res.status(404).json({error:"Document not found"});if(req.body.status)x.status=String(req.body.status);writeStore(s);res.json(x);}catch(e){res.status(403).json({error:e.message});}});
app.delete("/api/realestate/documents/:id",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);s.realEstate.documents=s.realEstate.documents.filter(x=>!(x.id===req.params.id&&x.clientId===id));writeStore(s);res.json({ok:true});}catch(e){res.status(403).json({error:e.message});}});

// ---- Broker commission & payout tracker ----
app.get("/api/realestate/commissions",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);res.json(s.realEstate.commissions.filter(x=>x.clientId===id).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt))));}catch(e){res.status(403).json({error:e.message});}});
app.post("/api/realestate/commissions",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);const bookingValue=Math.max(0,Number(req.body.bookingValue)||0),pct=Math.max(0,Number(req.body.commissionPercent)||0);const x={id:reId("comm"),clientId:id,leadId:req.body.leadId||null,partnerId:req.body.partnerId||null,partnerType:String(req.body.partnerType||"channel_partner"),bookingValue,commissionPercent:pct,commissionAmount:Math.round(bookingValue*pct/100),status:"pending",createdAt:reNow(),paidAt:null};s.realEstate.commissions.push(x);activity(s,id,"commission",`Commission of ₹${x.commissionAmount.toLocaleString("en-IN")} recorded`,{commissionId:x.id});writeStore(s);res.status(201).json(x);}catch(e){res.status(403).json({error:e.message});}});
app.patch("/api/realestate/commissions/:id",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s),x=s.realEstate.commissions.find(c=>c.id===req.params.id&&c.clientId===id);if(!x)return res.status(404).json({error:"Commission not found"});if(req.body.status){x.status=String(req.body.status);if(x.status==="paid")x.paidAt=reNow();}writeStore(s);res.json(x);}catch(e){res.status(403).json({error:e.message});}});
app.delete("/api/realestate/commissions/:id",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);s.realEstate.commissions=s.realEstate.commissions.filter(x=>!(x.id===req.params.id&&x.clientId===id));writeStore(s);res.json({ok:true});}catch(e){res.status(403).json({error:e.message});}});
...
app.get("/api/realestate/possession",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);res.json(s.realEstate.possession.filter(x=>x.clientId===id));}catch(e){res.status(403).json({error:e.message});}});
app.post("/api/realestate/possession",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s),l=leadById(s,req.body.leadId,id);if(!l)return res.status(404).json({error:"Lead not found"});const x={id:reId("poss"),clientId:id,leadId:l.id,projectId:req.body.projectId||null,unit:String(req.body.unit||""),expectedDate:String(req.body.expectedDate||""),stage:"BOOKING_CONFIRMED",notes:String(req.body.notes||"").slice(0,1000),createdAt:reNow(),updatedAt:reNow()};s.realEstate.possession.push(x);activity(s,id,"possession",`Post-sale tracking started for ${l.name}`,{leadId:l.id,possessionId:x.id});writeStore(s);res.status(201).json(x);}catch(e){res.status(403).json({error:e.message});}});
app.patch("/api/realestate/possession/:id",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s),x=s.realEstate.possession.find(p=>p.id===req.params.id&&p.clientId===id);if(!x)return res.status(404).json({error:"Record not found"});if(req.body.stage&&POSSESSION_STAGES.includes(req.body.stage)){x.stage=req.body.stage;activity(s,id,"possession",`Post-sale stage moved to ${x.stage.replaceAll("_"," ")}`,{leadId:x.leadId,possessionId:x.id});}if(req.body.expectedDate!==undefined)x.expectedDate=String(req.body.expectedDate);if(req.body.notes!==undefined)x.notes=String(req.body.notes).slice(0,1000);x.updatedAt=reNow();writeStore(s);res.json(x);}catch(e){res.status(403).json({error:e.message});}});

// ---- Testimonials & reviews ----
app.get("/api/realestate/testimonials",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);res.json(s.realEstate.testimonials.filter(x=>x.clientId===id).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt))));}catch(e){res.status(403).json({error:e.message});}});
app.post("/api/realestate/testimonials",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);const x={id:reId("rev"),clientId:id,leadId:req.body.leadId||null,name:String(req.body.name||"").trim().slice(0,80),rating:Math.min(5,Math.max(1,Number(req.body.rating)||5)),text:String(req.body.text||"").slice(0,1000),source:String(req.body.source||"Direct"),approved:false,createdAt:reNow()};if(!x.name||!x.text)return res.status(400).json({error:"Name and review text are required"});s.realEstate.testimonials.push(x);writeStore(s);res.status(201).json(x);}catch(e){res.status(403).json({error:e.message});}});
app.patch("/api/realestate/testimonials/:id",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s),x=s.realEstate.testimonials.find(t=>t.id===req.params.id&&t.clientId===id);if(!x)return res.status(404).json({error:"Review not found"});if(req.body.approved!==undefined)x.approved=!!req.body.approved;writeStore(s);res.json(x);}catch(e){res.status(403).json({error:e.message});}});
app.delete("/api/realestate/testimonials/:id",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);s.realEstate.testimonials=s.realEstate.testimonials.filter(x=>!(x.id===req.params.id&&x.clientId===id));writeStore(s);res.json({ok:true});}catch(e){res.status(403).json({error:e.message});}});
