...
// Staff (individual salesperson) sessions are restricted to their own
// assigned leads and locked out of management/financial endpoints — this
// is the actual enforcement boundary for that, not just hidden nav links.
function requireOwner(req){ if(req.session.user?.role==="staff") throw new Error("Only the business owner or an admin can access this."); }
// Returns the repId to scope by if this session is a staff login, else null
// (owner/admin sessions see everything, unscoped).
function staffScope(req){ return req.session.user?.role==="staff" ? req.session.user.repId : null; }
function actorName(req){ return req.session.user?.name||req.session.user?.email||null; }
...
app.post("/api/auth/login",rateLimit("login",10,60000),(req,res)=>{
  ...
  // scoped to only their own assigned leads (enforced below, not just
  // hidden in the UI), instead of the whole team sharing one password.
  for(const client of s.clients){
    const rep=(s.realEstate?.team||[]).find(t=>t.clientId===client.id&&t.loginEmail?.toLowerCase()===email&&t.passwordHash);
...
// Before this, a client's account stayed "active" forever after signup —
// nothing ever checked back in with Razorpay, so a failed renewal or a
// cancellation on Razorpay's side never reached AssistQ. This endpoint is
// what closes that gap: Razorpay calls it automatically the moment a real
// subscription event happens (charged, cancelled, halted, payment failed),
// and it flips the matching client's access on or off accordingly — no
// manual checking required. Configure this URL as a webhook in Razorpay
// Dashboard → Settings → Webhooks, subscribed to at least: subscription.
// charged, subscription.cancelled, subscription.halted, subscription.
// completed, subscription.activated, payment.failed. Set the same secret
// you choose there as RAZORPAY_WEBHOOK_SECRET here.
app.post("/api/billing/webhook", (req, res) => {
  try {
    ...
    if (secret) {
      ...
      if (got.length !== exp.length || !crypto.timingSafeEqual(got, exp)) return res.status(401).send("Invalid signature");
    }
    const event = String(req.body?.event || "");
    const subId = req.body?.payload?.subscription?.entity?.id || req.body?.payload?.payment?.entity?.subscription_id || null;
    if (!subId) return res.sendStatus(200); // nothing we can match this to — acknowledge so Razorpay doesn't keep retrying

    const s = ensureStoreShape(readStore());
    const client = s.clients.find(c => c.razorpaySubscriptionId === subId);
    if (!client) return res.sendStatus(200); // e.g. a manually-created client with no Razorpay subscription attached — nothing to do

    const CUT_OFF = ["subscription.cancelled", "subscription.halted", "subscription.completed", "payment.failed"];
    const RESTORE = ["subscription.activated", "subscription.charged", "subscription.resumed"];

    if (CUT_OFF.includes(event) && client.subscriptionStatus !== "suspended") {
      client.subscriptionStatus = "suspended";
      activity(s, client.id, "billing", `Access automatically suspended — Razorpay reported "${event}"`, { event, subscriptionId: subId });
    } else if (RESTORE.includes(event) && client.subscriptionStatus === "suspended") {
      client.subscriptionStatus = "active";
      activity(s, client.id, "billing", `Access automatically restored — Razorpay reported "${event}"`, { event, subscriptionId: subId });
    }
    writeStore(s);
    res.sendStatus(200);
  } catch (e) {
    console.error("ASSISTQ Razorpay webhook:", e.message);
    res.sendStatus(200); // always acknowledge, even on our own bug — Razorpay retries aggressively and we don't want a pile-up
  }
});
...
function parseHtml(html,url){const title=(html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1]||"").replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim();const description=(html.match(/<meta[^>]+name=["']description["'][^>]+content=["']([^"']*)["'][^>]*>/i)?.[1]||html.match(/<meta[^>]+content=["']([^"']*)["'][^>]+name=["']description["'][^>]*>/i)?.[1]||"").trim();const h1=(html.match(/<h1\b/gi)||[]).length;const h2=(html.match(/<h2\b/gi)||[]).length;const canonical=html.match(/<link[^>]+rel=["']canonical["'][^>]+href=["']([^"']*)["'][^>]*>/i)?.[1]||"";const imgs=[...html.matchAll(/<img\b[^>]*>/gi)].map(m=>m[0]);const missingAlt=imgs.filter(x=>!(/\balt\s*=\s*["'][^"']*["']/i.test(x))).length;const links=[...html.matchAll(/<a\b[^>]+href=["']([^"']+)["'][^>]*>/gi)].map(m=>absolute(url,m[1])).filter(Boolean);const jsonLd=(html.match(/<script[^>]+type=["']application\/ld\+json["']/gi)||[]).length;const viewport=/<meta[^>]+name=["']viewport["']/i.test(html);const noindex=/<meta[^>]+name=["']robots["'][^>]+content=["'][^"']*noindex/i.test(html)||/<meta[^>]+content=["'][^"']*noindex[^"']*["'][^>]+name=["']robots["']/i.test(html);const ogTitle=/<meta[^>]+property=["']og:title["']/i.test(html);const ogDescription=/<meta[^>]+property=["']og:description["']/i.test(html);const twitterCard=/<meta[^>]+name=["']twitter:card["']/i.test(html);const textOnly=html.replace(/<script[\s\S]*?<\/script>/gi," ").replace(/<style[\s\S]*?<\/style>/gi," ").replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim();const wordCount=textOnly?textOnly.split(/\s+/).length:0;const internalLinks=[...new Set(links.filter(x=>sameOrigin(x,url)))].slice(0,40);return {url,title,description,h1,h2,canonical,missingAlt,images:imgs.length,internalLinks,structuredData:jsonLd,https:url.startsWith("https://"),viewport,noindex,ogTitle,ogDescription,twitterCard,wordCount};}
...
async function runSeoAudit(inputUrl){let url=inputUrl.trim();if(!/^https?:\/\//i.test(url))url="https://"+url;const root=new URL(url);const seen=new Set(),queue=[root.toString()];const pages=[],broken=[];let robots=false,sitemap=false;try{const rr=await fetchText(new URL("/robots.txt",root).toString(),5000);robots=rr.ok;const sm=rr.text.match(/Sitemap:\s*(\S+)/i)?.[1];if(sm){const sr=await fetchText(sm,5000);sitemap=sr.ok;}else{const sr=await fetchText(new URL("/sitemap.xml",root).toString(),5000);sitemap=sr.ok;}}catch{}while(queue.length&&pages.length<10){const u=queue.shift();if(seen.has(u))continue;seen.add(u);try{const r=await fetchText(u,8000);if(!r.ok){broken.push({url:u,status:r.status});continue;}const p=parseHtml(r.text,r.url);pages.push({...p,status:r.status});for(const link of p.internalLinks){if(pages.length+queue.length>=15)break;const nu=new URL(link);nu.hash="";if(!seen.has(nu.toString())&&!queue.includes(nu.toString()))queue.push(nu.toString());}}catch(e){broken.push({url:u,status:e.name==="AbortError"?"timeout":"error"});}}const checks=[];const add=(name,ok,detail,weight="warning")=>checks.push({name,ok,detail,weight});const noTitle=pages.filter(p=>!p.title||p.title.length<10||p.title.length>65).length;const noDesc=pages.filter(p=>!p.description||p.description.length<50||p.description.length>170).length;const badH1=pages.filter(p=>p.h1!==1).length;const badCanonical=pages.filter(p=>!p.canonical).length;const alt=pages.reduce((n,p)=>n+p.missingAlt,0);const noViewport=pages.filter(p=>!p.viewport).length;const noindex=pages.filter(p=>p.noindex).length;const noOg=pages.filter(p=>!p.ogTitle||!p.ogDescription).length;const noTwitter=pages.filter(p=>!p.twitterCard).length;const thin=pages.filter(p=>p.wordCount<300).length;const fewLinks=pages.filter(p=>p.internalLinks.length<2).length;const noSchema=pages.filter(p=>p.structuredData===0).length;add("HTTPS",root.protocol==="https:",root.protocol==="https:"?"Secure connection detected":"Use HTTPS for the client website","critical");add("Robots.txt",robots,robots?"Robots file found":"robots.txt not found or unavailable");add("XML sitemap",sitemap,sitemap?"Sitemap discovered":"No XML sitemap discovered");add("Title tags",noTitle===0,`${noTitle} of ${pages.length} pages need title attention`);add("Meta descriptions",noDesc===0,`${noDesc} of ${pages.length} pages need description attention`);add("H1 structure",badH1===0,`${badH1} of ${pages.length} pages do not have exactly one H1`);add("Canonical tags",badCanonical===0,`${badCanonical} of ${pages.length} pages have no canonical`);add("Image alt text",alt===0,alt?`${alt} images missing alt text`:"All sampled images have alt text");add("Structured data",noSchema===0,`${noSchema} of ${pages.length} pages have no JSON-LD`);add("Mobile viewport",noViewport===0,`${noViewport} of ${pages.length} pages are missing a viewport meta tag`);add("Indexability",noindex===0,`${noindex} sampled pages contain a noindex directive`,"critical");add("Social sharing tags",noOg===0,`${noOg} pages need Open Graph title/description`);add("Twitter card",noTwitter===0,`${noTwitter} pages are missing a Twitter card`);add("Content depth",thin===0,`${thin} pages have less than 300 words of crawlable text`);add("Internal linking",fewLinks===0,`${fewLinks} pages have fewer than 2 internal links`);add("Broken internal links",broken.length===0,broken.length?`${broken.length} broken/unreachable URLs found in sampled crawl`:"No broken URLs found in sampled crawl","critical");const critical=checks.filter(c=>!c.ok&&c.weight==="critical").length;const warnings=checks.filter(c=>!c.ok&&c.weight!=="critical").length;const score=Math.max(0,Math.round(100-(critical*15)-(warnings*5)));const recommendations=[];const push=(level,title,action)=>{if(recommendations.length<12)recommendations.push({level,title,action});};if(!robots)push("warning","Add robots.txt","Publish a crawl-control file so search engines can discover your sitemap and follow your preferred crawl rules.");if(!sitemap)push("warning","Add an XML sitemap","Create and submit a sitemap containing important indexable pages.");if(noTitle)push("critical","Improve page titles","Give every important page a unique, descriptive title around the search intent it should rank for.");if(noDesc)push("warning","Improve meta descriptions","Write useful, unique descriptions that match the page and encourage qualified clicks.");if(badH1)push("warning","Fix H1 structure","Use one clear primary heading per important page and align it with the page's search intent.");if(noindex)push("critical","Review noindex pages","Some sampled pages tell search engines not to index them. Keep noindex only where intentional.");if(noSchema)push("warning","Add structured data","Use relevant Schema.org markup where appropriate, such as Organization, LocalBusiness, Product or BreadcrumbList.");if(thin)push("warning","Strengthen thin pages","Add useful buyer-focused information, FAQs, location details and clear calls to action instead of filler text.");if(fewLinks)push("warning","Improve internal links","Link related service, location and property pages together so visitors and crawlers can discover important content.");if(noOg)push("info","Improve social sharing","Add Open Graph title and description so shared pages look professional on social platforms.");if(alt)push("warning","Add image alt text","Describe meaningful images for accessibility and better image-search context.");if(broken.length)push("critical","Fix broken URLs","Repair or redirect broken internal links before sending more traffic to the site.");const avgWords=pages.length?Math.round(pages.reduce((n,p)=>n+p.wordCount,0)/pages.length):0;const avgInternalLinks=pages.length?Math.round(pages.reduce((n,p)=>n+p.internalLinks.length,0)/pages.length):0;return {url,checkedAt:new Date().toISOString(),pagesChecked:pages.length,score,critical,warnings,checks,pages:pages.map(p=>({url:p.url,status:p.status,title:p.title,description:p.description,h1:p.h1,h2:p.h2,missingAlt:p.missingAlt,canonical:p.canonical,structuredData:p.structuredData,wordCount:p.wordCount})),broken,recommendations,indexability:{indexablePages:pages.filter(p=>!p.noindex).length,noindexPages:noindex,canonicalPages:pages.filter(p=>!!p.canonical).length},content:{avgWords,avgInternalLinks},structuredDataPages:pages.filter(p=>p.structuredData>0).length};}
...
}if(req.body.assignedTo!==undefined){requireOwner(req);l.assignedTo=req.body.assignedTo||null;activity(s,id,"assignment",`${l.name} assigned to ${l.assignedTo||"unassigned"}`,{leadId:l.id},actor);}if(req.body.notes!==undefined)l.notes=String(req.body.notes).slice(0,5000);if(req.body.responseMinutes!==undefined)l.responseMinutes=Math.max(0,Number(req.body.responseMinutes)||0);l.updatedAt=reNow();writeStore(s);res.json(enrichLead(l,s));}catch(e){res.status(403).json({error:e.message});}});
app.get("/api/realestate/team",requireAuth,(req,res)=>{try{const s=ensureStoreShape(readStore()),id=reClient(req,s);if(req.session.user?.role!=="admin"&&!planHasFeature(clientPlanTier(s,id),"team"))return res.status(403).json({error:"Sales team management requires the Premium plan."});res.json(s.realEstate.team.filter(x=>x.clientId===id).map(t=>{const{passwordHash,...safe}=t;return {...safe,hasLogin:!!passwordHash};}));}catch(e){res.status(403).json({error:e.message});}});
app.post("/api/realestate/team",requireAuth,(req,res)=>{try{requireOwner(req);const s=ensureStoreShape(readStore()),id=reClient(req,s);if(req.session.user?.role!=="admin"&&!planHasFeature(clientPlanTier(s,id),"team"))return res.status(403).json({error:"Sales team management requires the Premium plan."});const t={id:reId("rep"),clientId:id,name:String(req.body.name||"").trim(),phone:String(req.body.phone||""),email:String(req.body.email||""),areas:Array.isArray(req.body.areas)?req.body.areas:String(req.body.areas||"").split(",").map(x=>x.trim()).filter(Boolean),minBudget:String(req.body.minBudget||""),maxBudget:String(req.body.maxBudget||""),active:req.body.active!==false};if(!t.name)return res.status(400).json({error:"Salesperson name required"});s.realEstate.team.push(t);writeStore(s);res.status(201).json(t);}catch(e){res.status(403).json({error:e.message});}});
app.delete("/api/realestate/team/:id",requireAuth,(req,res)=>{try{requireOwner(req);const s=ensureStoreShape(readStore()),id=reClient(req,s);if(req.session.user?.role!=="admin"&&!planHasFeature(clientPlanTier(s,id),"team"))return res.status(403).json({error:"Sales team management requires the Premium plan."});s.realEstate.team=s.realEstate.team.filter(x=>!(x.id===req.params.id&&x.clientId===id));writeStore(s);res.json({ok:true});}catch(e){res.status(403).json({error:e.message});}});
// Gives one salesperson their own login, separate from the client's single
// shared account — this is what makes individual accountability possible.
// Owner/admin only, on purpose: a rep can't grant themselves or a colleague
// access. Returns the plaintext password exactly once (same pattern as a
// client's access code) — it is never stored or retrievable again, only
// its hash, so the owner must actually hand it to the rep now.
app.post("/api/realestate/team/:id/login",requireAuth,(req,res)=>{try{requireOwner(req);const s=ensureStoreShape(readStore()),id=reClient(req,s);if(req.session.user?.role!=="admin"&&!planHasFeature(clientPlanTier(s,id),"team"))return res.status(403).json({error:"Individual salesperson logins require the Premium plan."});const t=s.realEstate.team.find(x=>x.id===req.params.id&&x.clientId===id);if(!t)return res.status(404).json({error:"Salesperson not found"});const loginEmail=String(req.body.email||t.email||"").trim().toLowerCase();if(!loginEmail)return res.status(400).json({error:"An email is required to set up this salesperson's login."});if(s.realEstate.team.some(x=>x.id!==t.id&&x.loginEmail?.toLowerCase()===loginEmail))return res.status(409).json({error:"Another salesperson already uses this email to log in."});const password=req.body.password?String(req.body.password):crypto.randomBytes(5).toString("hex").toUpperCase();t.loginEmail=loginEmail;t.passwordHash=hashPassword(password);writeStore(s);res.json({ok:true,loginEmail,password});}catch(e){res.status(403).json({error:e.message});}});
...
app.get("/api/realestate/partners",requireAuth,(req,res)=>{try{requireOwner(req);const s=ensureStoreShape(readStore()),id=reClient(req,s);res.json(s.realEstate.channelPartners.filter(x=>x.clientId===id));}catch(e){res.status(403).json({error:e.message});}});
app.post("/api/realestate/partners",requireAuth,(req,res)=>{try{requireOwner(req);const s=ensureStoreShape(readStore()),id=reClient(req,s);const x={id:reId("cp"),clientId:id,name:String(req.body.name||"").trim(),phone:String(req.body.phone||""),email:String(req.body.email||""),company:String(req.body.company||""),status:"active",createdAt:reNow()};if(!x.name)return res.status(400).json({error:"Partner name required"});s.realEstate.channelPartners.push(x);writeStore(s);res.status(201).json(x);}catch(e){res.status(403).json({error:e.message});}});
